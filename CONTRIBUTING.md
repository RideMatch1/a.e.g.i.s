...
pnpm install
pnpm -r build
pnpm -r test                     # expect 3055 green across 7 packages
node packages/benchmark/run.mjs  # expect 30/30 strict
```
...
- **Benchmark green before push**: `node packages/benchmark/run.mjs` must stay 30/30 strict. Changes that shift the count require an `expected.json` update in the same commit.
