...
| SnailSploit/The-LLM-Red-Teamer's-Playbook | [SnailSploit/The-LLM-Red-Teamer-s-Playbook](https://github.com/SnailSploit/The-LLM-Red-Teamer-s-Playbook) | CC-BY-SA-4.0 | Defense-layer diagnostic methodology (latency-fingerprinting, identity-displacement, RAG-poisoning, agentic-trust-boundary). Concept-only inspiration for v0.18.0 F-PROMPT-AGENT-LOOP-1 (`messages.push()` tool-feedback detection in prompt-injection-checker). Same share-alike-vs-MIT incompatibility as Trail of Bits Skills entry above — no upstream prose or payloads copied; AEGIS-side patterns are original. |
| PurpleAILAB/Decepticon | [PurpleAILAB/Decepticon](https://github.com/PurpleAILAB/Decepticon) | Apache-2.0 | Multi-agent kill-chain orchestration architecture (17 specialist agents organized by ATT&CK phase, two-network sandbox isolation, persistent state via Neo4j). Architectural reference for `aegis siege` phase-orchestration; full-platform integration intentionally not pursued (Docker + Kali sandbox + LiteLLM + management network requirements are out of scope for AEGIS' discrete-scanner model). No code copied. |
| zakirkun/deep-eye | [zakirkun/deep-eye](https://github.com/zakirkun/deep-eye) | MIT | Multi-LLM-DAST tool (OpenAI/Claude/Grok/OLLAMA/OpenRouter). Wrapper integration intentionally not pursued — 7/11 modules redundant with AEGIS' existing Strix / PTAI / Pentest-Swarm-AI / ZAP / Nuclei wrappers, plus 3-month-stale upstream and stack mismatch (Python vs AEGIS' TS). Module set validated three real AEGIS gaps that were already roadmap-tracked: (1) Slack/Discord notification channels — closes APTS-HO-015 partial multi-channel TODO via F-NOTIFY-CHANNELS-1, (2) WebSocket DAST scanner — gap, tracked as F-WEBSOCKET-DAST-1, (3) GraphQL programmatic DAST — gap, tracked as F-GRAPHQL-DAST-1. No code copied; AEGIS-side implementations are original. |
