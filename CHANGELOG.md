...
- **22 unit tests + 14 canary fixtures (6 TP + 8 FP)**. After the initial commit, a battle-test on LO's customer codebases (seitengold, hundementor, hundementor-app, Spa-App, hotel-pilot-landing, UCOS-final-sp1) plus an AEGIS self-scan surfaced three real-world FP classes that the initial regex set missed. Three regression-guard fixtures + matching unit tests were added; the regexes were tightened: (1) IIFE detector now anchored at expression-statement-start so `getRequestConfig(async () => { ... })`-style host-callbacks (Spa-App next-intl pattern) are no longer misclassified as IIFEs; (2) `(?<!function\s)` lookbehind on spawn/exec/eval/require/setInterval skips function definitions like `export function exec(...)` (AEGIS utils.ts pattern); (3) `(?<!\.)` lookbehind skips method calls like `regex.exec(args)` (AEGIS mass-assignment-checker pattern). Bare `exec(` is no longer detected on its own — too ambiguous; only the explicit `child_process.exec(` namespace fires for `exec`. Post-fix battle-test: 0 findings on all 6 LO customer projects; AEGIS self-scan flagged only the 6 v0181 TP fixtures (correct — those ARE the test fixtures by design).
