...
- **Four new sub-class detectors closing the M-01 narrow-sanitizer family.** External Field-Report (`docs/AEGIS-Field-Report-2026-04-27.md`) by Alexander Hertle (Neon Arc / UCOS Engine) — production-deployed AEGIS user since v0.15.6 — empirically demonstrated four detector sub-classes that the existing `prompt-injection-checker` missed against a Mistral-backed production chatbot. All four are now flagged statically, with both positive- and negative-fixture coverage end-to-end.
  ...
  - **End-to-end fixtures from public neonarc branch.** Pre-fix sanitiser fixture (the field-report's vulnerable shape) produces ≥1 finding per Sub-Klasse 1+2+3+4. Post-fix sanitiser fixture (sourced verbatim from `https://github.com/ephixa53/neonarc/tree/security/chatbot-prompt-injection-hardening` path `src/lib/chat/sanitize.ts` — the production hardened version) produces ZERO weak-defense findings. Fixture files use `.txt` extension to opt out of `walkFiles` extension filter; written into temp project as `.ts` only at test runtime. Attribution headers on each fixture credit the original author and source branch.
  ...
  - **Acknowledgment:** Alexander Hertle / Neon Arc / UCOS Engine (`info@neonarc.com`). Repro anchors per Beobachtung captured in the `WEAK_DEFENSE_PATTERNS` rule descriptions; full report preserved at `docs/AEGIS-Field-Report-2026-04-27.md`. Source-of-truth corpus branch: `github.com/ephixa53/neonarc:security/chatbot-prompt-injection-hardening`.
...
- **6 audit-flagged gaps closed end-to-end with live empirical evidence.** A brutal internal re-audit on 2026-04-27 found 6 substantive gaps in the Phase-2 work and the conformance posture. Each is now closed with code, tests, and live verification rather than paper rationale.
  ...
  5. **Hash-chain survives signal-kill end-to-end.** Previously a signal-emitted kill event bypassed ChainedEmitter and broke `audit-verify` with a "missing this_hash" error at the kill line. New: `installSignalHandlers` accepts an optional `chainEmitter` and routes the kill/intervention event through it; siege passes its ChainedEmitter at install time. Verified live: full `aegis siege --state-file ... --allow-loopback` against Spa-App + hundementor, then SIGTERM, then `aegis audit-verify` reports "chain intact, 8 events, APTS-AR-012 conformant" + tampering test (mutating one event line) correctly localizes the broken_at line.

  6. **`--allow-loopback` opt-in flag for legitimate local-pentest workflows.** safeFetch correctly rejects 127.x.x.x + ::1 by default (SSRF defense). Previously this made siege against local dev servers (Spa-App on :3001, hundementor on :3007) impossible without bypassing safeFetch. New: `--allow-loopback` CLI flag + `SafeFetchOptions.allowLoopback` opt-in. When enabled, safeFetch reclassifies loopback resolutions as 'public'; private-IP / link-local / cloud-metadata rejections are NOT bypassed (operator can only opt into loopback specifically). Emits a yellow stderr warning + an `operator-acknowledged-loopback` audit event into the chain. 7 new core unit tests cover IPv4 + IPv6 loopback, DNS-resolved loopback, and verify NO bypass on private-ip / cloud-metadata / link-local.
