...
### Added (compliance posture, post-v0.16.6)

- **OWASP-APTS Conformance Posture (Phase 1 — Tier-1 Readiness Assessment)** — first OSS pentest platform with a published APTS conformance posture. Phase 1 is explicitly a Readiness Assessment / gap statement, **not** a conformance claim, because APTS forbids partial credit (a Tier-1 claim requires 100% MET on the claimed tier). All 72 Tier-1 requirements explicitly classified (`met`/`partially_met`/`not_met`/`not_applicable`/`planned`), schema-valid against the upstream `apts_requirements.json` v0.1.0 manifest. Headline: 15/72 MET (21%), with concrete Phase-2 closure plans for the remaining 57 gaps. Boundary scope: AEGIS Autonomous Pentest Layer = `aegis siege` + LLM-pentest wrappers (Strix/PTAI/Pentest-Swarm-AI) + CLI orchestration; deterministic SAST scanners, `@aegis-scan/skills`, and `@aegis-wizard/cli` are supporting components and out-of-scope. Phase 1 deliverables (doc-only, zero blast-radius) under `docs/compliance/owasp-apts/`: README, CONFORMANCE-CLAIM, conformance.json, EVIDENCE-MANIFEST, FOUNDATION-MODEL-DISCLOSURE, gap-summary, attribution. CC BY-SA 4.0 license terms preserved per upstream OWASP/APTS share-alike. Spec: `docs/design/2026-04-27-owasp-apts-conformance.md`. Implementation plan: `docs/design/2026-04-27-owasp-apts-conformance-plan.md`.
